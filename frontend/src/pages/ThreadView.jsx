...
export default function ThreadView() {
  ...
  async function refreshAi() {
    if (!latest) return;
    await crmApi.analyze({
      sender: latest.sender,
      subject: latest.subject,
      body: latest.body,
      email_id: latest.id,
      thread_history: data.emails.map((email) => email.body),
      dry_run: true,
    });
    reload();
  }
      ...
      <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
        <section className="space-y-3">
          {data.emails.map((email) => (
            <article key={email.id} className="rounded border border-slate-200 bg-white p-4">
        ...
        <aside className="space-y-4">
          ...
          <Panel title="Classification">
            {latest?.classification ? `${latest.classification.category} · ${latest.classification.urgency || 'No urgency'} · ${latest.classification.confidence}` : 'Not classified'}
          </Panel>
